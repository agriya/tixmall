 ...
 *
 * Date: Thu May 12 2016 06:01 EDT-0400
    ...
    (function(factory) {
        ...
        (function($) {
            ...
            var TRUE = true,
                ...
                BROWSER = {
                    ...
                    ie: (function() {
                        ...
                        var v, i;
                        for (v = 4, i = document.createElement('div');
                ...
                if ('content' in opts) {
                    ...
                    if ('ajax' in content) {
                        ...
                        content.text = function(event, api) {
                            var loading = text || $(this)
                                .attr(api.options.content.attr) || 'Loading...',
            ...
            CHECKS = PROTOTYPE.checks = {
                builtin: {
                    ...
                    '^style.classes$': function(obj, o, v, p) {
                        this.rendered && this.tooltip.removeClass(p)
                            .addClass(v);
                    },
            ...
            PROTOTYPE._update = function(content, element) {
                ...
                return this._waitForContent(element)
            ...
            PROTOTYPE._waitForContent = function(element) {
                ...
                return ($.fn.imagesLoaded ? element.imagesLoaded() : new $.Deferred()
                        .resolve([]))
                    .done(function() {
                        cache.waiting = FALSE;
                    })
            ...
            PROTOTYPE._createTitle = function() {
                ...
                elements.titlebar = $('<div />', {
                    ...
                    .delegate('.qtip-close', 'mousedown keydown mouseup keyup mouseout', function(event) {
                        $(this)
                            .toggleClass('ui-state-active ui-state-focus', event.type.substr(-4) === 'down');
                    })
                    .delegate('.qtip-close', 'mouseover mouseout', function(event) {
                        $(this)
                            .toggleClass('ui-state-hover', event.type === 'mouseover');
                    });
            ...
            PROTOTYPE._createPosClass = function(my) {
                return NAMESPACE + '-pos-' + (my || this.options.position.my)
                    .abbrev();
            };
            PROTOTYPE.reposition = function(event, effect) {
                ...
                if (cache.posClass !== (newClass = this._createPosClass(this.position.my))) {
                    ...
                    tooltip.removeClass(cache.posClass)
                        .addClass(newClass);
                }
                // tooltipmove event
            ...
            PROTOTYPE.toggle = function(state, event) {
                ...
                if (event) {
                    if ((/over|enter/)
                        .test(event.type) && cache.event && (/out|leave/)
                        .test(cache.event.type) && options.show.target.add(event.target)
                        .length === options.show.target.length && tooltip.has(event.relatedTarget)
                        .length) {
                        return this;
                    }
                    // Cache event
                ...
                if (state) {
                    ...
                    if (!trackingBound && posOptions.target === 'mouse' && posOptions.adjust.mouse) {
                        $(document)
                            .bind('mousemove.' + NAMESPACE, this._storeMouse);
                        trackingBound = TRUE;
                    }
                    ...
                    if (!!opts.solo) {
                        ...
                        .not(tooltip)
                            .not(opts.target)
                            .qtip('hide', new $.Event('tooltipsolo'));
                    }
                } else {
                    ...
                    if (trackingBound && !$(SELECTOR + '[tracking="true"]:visible', opts.solo)
                        .not(tooltip)
                        .length) {
                        $(document)
                ...
                after = $.proxy(function() {
                    if (state) {
                        ...
                        if ('string' === typeof opts.autofocus) {
                            $(this.options.show.autofocus, tooltip)
                                .focus();
                        }
            ...
            function hideMethod(event) {
                ...
                if (this !== relatedTarget[0] && (this.options.position.target === 'mouse' && ontoTooltip) || this.options.hide.fixed && (
            ...
            PROTOTYPE._bind = function(targets, events, method, suffix, context) {
                ...
                $(targets)
                    .bind(
                        (events.split ? events : events.join(ns + ' ')) + ns, $.proxy(method, context || this));
                return this;
            };
            PROTOTYPE._unbind = function(targets, suffix) {
                targets && $(targets)
                    .unbind('.' + this._id + (suffix ? '-' + suffix : ''));
                return this;
            };
            // Global delegation helper
            function delegate(selector, events, method) {
                $(document.body)
                    .delegate(selector, (events.split ? events : events.join('.' + NAMESPACE + ' ')) + '.' + NAMESPACE, function() {
            ...
            PROTOTYPE._bindEvents = function(showEvents, hideEvents, showTargets, hideTargets, showCallback, hideCallback) {
                ...
                var similarTargets = showTargets.filter(hideTargets)
                ...
                if (similarTargets.length) {
                    ...
                    if (toggleEvents.length) {
                        ...
                        this._bind(similarTargets, toggleEvents, function(event) {
                            ...
                            (state ? hideCallback : showCallback)
                            .call(this, event);
                        });
                ...
                if (('' + options.hide.event)
                    ...
                    this._bind(containerTarget.closest('html'), ['mousedown', 'touchstart'], function(event) {
                        var elem = $(event.target),
                            ...
                            isAncestor = elem.parents(SELECTOR)
                            .filter(this.tooltip[0])
                            .length > 0;
                        if (elem[0] !== this.target[0] && elem[0] !== this.tooltip[0] && !isAncestor && !this.target.has(elem[0])
                            .length && enabled) {
                            this.hide(event);
                        }
                    });
            ...
            $(function() {
                delegate(SELECTOR, ['mouseenter', 'mouseleave'], function(event) {
                    ...
                    // On mouseleave...
                        ...
                        if (options.position.target === 'mouse' && options.position.adjust.mouse && options.hide.event && options.show.target && !target.closest(options.show.target[0])
                            .length) {
                            this.hide(event);
                        }
                    }
            ...
            PLUGINS.svg = function(api, svg, corner) {
                ...
                switch (elem.nodeName) {
                    case 'ellipse':
                    case 'circle':
                        result = PLUGINS.polys.ellipse(elem.cx.baseVal.value, elem.cy.baseVal.value, (elem.rx || elem.r)
                            .baseVal.value + strokeWidth2, (elem.ry || elem.r)
                            .baseVal.value + strokeWidth2, corner);
                        break;
                    case 'line':
                    case 'polygon':
                    case 'polyline':
                ...
                if (ownerDocument !== document && api.position.target !== 'mouse') {
                    frameOffset = $((ownerDocument.defaultView || ownerDocument.parentWindow)
                            .frameElement)
                        .offset();
                    if (frameOffset) {
